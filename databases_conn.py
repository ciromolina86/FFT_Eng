...
class DBmysql:
    ...
    def __init__(self, info):
        ...
        while (db_retry is True) and (db_retry_counter <= max_retry):
            try:
                ...
                # Try to connect to MySQL database again
                db_retry = True
                db_retry_counter += 1
...
class VibModel:
    ...
    def __init__(self):
        ...
        if self.mysql_conn.cursor is not None:
            self.model = self.get_model()
        else:
            print('>>>> MySQL database cursor is not connected. Check MySQL database connection!')
            pass
